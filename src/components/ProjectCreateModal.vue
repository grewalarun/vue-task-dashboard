      ...
      >
        <div class="bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-md max-h-[90vh] overflow-y-auto">
          <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 sticky top-0 bg-white z-10">
            <h3 class="text-sm font-semibold text-slate-900">Add New Project</h3>
          ...
          <div class="px-6 py-5 space-y-4">
            <div class="space-y-1.5">
              ...
              <input v-model="form.name" type="text" placeholder="e.g. Website Redesign" class="w-full px-3.5 py-2.5 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all" :class="errors.name ? 'border-rose-300' : 'border-slate-200'" />
            ...
            <div class="space-y-1.5">
              ...
              <textarea v-model="form.description" rows="3" placeholder="Optional project details..." class="w-full px-3.5 py-2.5 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all resize-none"></textarea>
            </div>
          </div>
          ...
          <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/50 rounded-b-2xl">
            ...
            <div class="flex justify-end gap-3">
              ...
              >
                ...
                {{ projectStore.loading ? 'Saving…' : 'Create Project' }}
              </button>
            </div>
          </div>
        </div>
...
async function submit() {
  ...
  if (!projectStore.error) {
    // Optionally fetch projects again to update the sidebar list
    await projectStore.fetchProjects()
    close()
    
    // Navigate to the newly created project dashboard
    if (projectStore.project && projectStore.project._id) {
      router.push(`/projects/${projectStore.project._id}`)
    }
  }
}
</script>
